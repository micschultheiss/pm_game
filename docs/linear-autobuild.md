...
card to **Deploy** and Claude ships it to prod. This doc explains the moving
...
- **In Review is the human gate.** Claude stops there and comments the commits,
  the staging URL it verified, and a short description. You test on
  `https://hallucination-inc-staging.fly.dev/`.
- **Deploy = ship.** Moving a card into the Deploy column is your approval:
...
action moves the card *out* of its trigger column, so the watcher is naturally
idempotent — no local state to track.
...
into Todo (or Deploy) is itself the trigger, it fires within one poll interval of
...
| [`scripts/linear_watch.py`](../scripts/linear_watch.py) | Pure classifier. Reads the board (via the caller) and returns `to_build` (cards in Todo) / `to_ship` (cards in Deploy). Stateless; no git/Fly/build side effects. |
...
1. **The `Deploy` column** (the ship trigger) must exist in the workflow —
   **Linear → Settings → Team `Mics-playground` → Workflow** (already present as a
   Started-group state). `Todo` and `In Review` are stock states.
2. **Authenticate `fly`** so staging/prod deploys work: `fly auth whoami`
   (run `fly auth login` if it errors).
3. That's it — the classifier is stateless, nothing to seed.
...
- A build that fails tests stays in **In Progress** with a failure comment; it is
  never advanced to In Review and broken code is never committed.
