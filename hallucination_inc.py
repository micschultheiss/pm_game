...
Wraps the pure game logic in `engine.py` with an ANSI-coloured terminal UI:
banner header, action menus, prompts, end screens, and the blocking REPL
that ticks the day forward. Game state, actions, and oracles live in the
engine module — this file is presentation only.
...
# Re-export the engine surface so tests and simulators that import
# `hallucination_inc` keep working while the split settles in.
# `from engine import *` skips underscore-prefixed names, so private helpers
# (event applicators, recipe size, template lookup) are pulled explicitly.
from engine import *  # noqa: F401,F403
from engine import (
    # UI-facing public symbols
    ACTIVE_CLIENT_COUNT,
    DEBT_FREE_BONUS,
    MAX_DAYS,
    MAX_TOKENS,
    ...
    token_free,
    token_total,
    # Private helpers re-exported so the existing test suite keeps working
    # against `import hallucination_inc as g`. Drop these once tests target
    # `engine` directly.
